...
import { TextField, Button, Grid, Card, CardContent, Typography, Container, Stack } from '@mui/material';
...
const BookList: React.FC = () => {
  ...
  const fetchBooks = async (page: number, limit:number, searchQuery: string) => {
    const { data } = await axiosInstance.get('/api/v1/books', { params: { page, search: searchQuery, limit } });
    setBooks(data.data);
  ...
  return (
    <Container>
      ...
      <Stack direction={'row'} justifyContent={'space-between'}>
        ...
        <Button href='/add' variant='contained' sx={{height:'40px'}}>+ Create</Button>
      </Stack>
      <TextField
